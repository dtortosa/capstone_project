...
The goal of this project is to improve the prediction of EUR/USD exchange rate using the previous Twitter sentiment around these currencies. This was my capstone project for the Data Scientist Certification I completed at the Data Incubator ([Pragmatic Institue](https://www.credly.com/badges/f789cc0f-a7fc-4c04-adaf-8b72ca5f421c)).
...
The EUR/USD exchange ratio, the EUR-pricing predictors and the Twitter sentiment predictors were all merged into one single dataset, having one value of each variable per day. This included data for all the variables over the past **9 years**. I used this dataset to model the EUR/USD exchange rate of a given day as a function of the previous EUR pricing and Twitter sentiment. 
...
All these regressors were run with default parameters being extra trees regressors, the ones showing the highest R<sup>2</sup> for the test sets in general. Therefore, this regressor was used in subsequent analyses.
...
When predicting in the test sets, the extra tree regressors showed a very high predictive power with the full model (EUR pricing + Twitter sentiment) having the highest R<sup>2</sup> (R<sup>2</sup> = 0.99665). This suggests that the performance of the Twitter-based models is not caused just by an increase of predictors and overfitting. Note that this predictive power was still below the simplest model. However, the extra tree regressors surpassed the simplest model when fitted to the whole dataset, having again the full model the highest predictive power (R<sup>2</sup> = 0.9993), followed by the Twitter-only model (R<sup>2</sup> = 0.9989) and the EUR-only model (R<sup>2</sup> = 0.9982). It is worth to highlight that adding the Twitter sentiment increased the R<sup>2</sup> by **0.11%** with respect to the EUR-only model, a small increase but still relevant given the already high predictive power used as baseline. In addition, the model considering **only** the Twitter sentiment in the previous 15 days had a remarkable high predictive power (R<sup>2</sup> = 0.9989).
